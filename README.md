...
jobs:
  claude-response:
    ...
    steps:
      - uses: tsuhha/claude-code-action@beta  # Fork with OAuth support
        with:
...
steps:
  - uses: tsuhha/claude-code-action@beta
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`tsuhha/claude-code-base-action`](https://github.com/tsuhha/claude-code-base-action).
